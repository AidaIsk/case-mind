...
def _render_progress_summary():
    """Компактный блок прогресса над историей прогонов."""
    ...
    with col1:
        st.metric("Прогонов", summary["total_runs"])
    with col2:
        st.metric("Средний score", f"{summary['avg_score']} / 100")
    with col3:
        st.metric("Верных решений", f"{summary['correct_decision_rate']}%")
    with col4:
        st.metric("Тренд", _TREND_ICON.get(summary["score_trend"], summary["score_trend"]))
    ...
    if dist:
        ...
        st.caption(f"Частые причины ошибок: {causes_str}")
